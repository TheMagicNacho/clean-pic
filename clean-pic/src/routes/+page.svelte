<script lang="ts">
  ...
  const buttonDescriptions: Record<string, string> = {
    ...
    "scrub-view":
      "Switch to scrub mode to scrub out all that pesky metadata from your images!",
    "inspect-view":
      "Let's switch to inspect mode and take a peek at that sweet sweet image data!",
  };
  ...
  const reactionScript: Record<string, string> = {
    ...
    thinking: "hummmmm...",
    "what-i-found":
      "Look, Daddy! Here's what I found hiding inside your image!",
  };
...
<style>
  ...
  button {
    ...
    background: linear-gradient(
      135deg,
      var(--kawaii-hot-pink) 0%,
      var(--kawaii-bright-pink) 100%
    );
    transition: all 0.3s ease;
    box-shadow: 0 4px 15px var(--kawaii-shadow);
    position: relative;
  ...
  :root {
    ...
    font-family: "Nunito", "Comic Neue", Inter, Avenir, Helvetica, Arial,
      sans-serif;
    font-size: 16px;
    line-height: 24px;
    font-weight: 600;
